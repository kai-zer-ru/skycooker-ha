...
def parse_status_from_string(status_string):
    ...
    try:
        ...
        _LOGGER.info("Разобранный статус:")
        _LOGGER.info("  Режим: %s %s", mode_name, mode)
        _LOGGER.info("  Подрежим: %s", subprog)
        _LOGGER.info("  Целевая температура: %s C", target_temp)
        _LOGGER.info("  Время: %s часов %s минут", hours, minutes)
        _LOGGER.info("  Оставшееся время: %s часов %s минут", remaining_hours, remaining_minutes)
        _LOGGER.info("  Автоподогрев: %s", "Включен" if auto_warm else "Выключен")
        _LOGGER.info("  Статус: %s - %s", status_name, status)

        return result
    except Exception as e:
        _LOGGER.error("Ошибка при разборе строки статуса: %s", e)
        return None
...
async def get_version(library, mac, key):
    ...
    if result:
        _LOGGER.info("Версия мультиварки: %s", result)
    else:
    ...
    if continuous:
        ...
        try:
            while True:
                ...
                if result:
                    _LOGGER.info("Статус мультиварки: %s", result)
                else:
...
async def set_mode_default(library, mac, key, mode):
    ...
    Args:
        mode (int): Режим работы мультиварки.
    """
    _LOGGER.info("Установка режима %s с параметрами по умолчанию...", mode)
    result = await library.set_mode_default(mode)
    if result:
        _LOGGER.info("Режим %s установлен с параметрами по умолчанию", mode)
    else:
...
async def set_delayed_mode(library, mac, key, mode, subprog=0, target_temp=0, hours=0, minutes=0, dhours=0, dminutes=0, heat=0):
    ...
    Args:
        ...
        heat (int): Нагрев.
    """
    _LOGGER.info("Установка режима %s с отложенным временем...", mode)
    result = await library.set_delayed_mode(mode, subprog, target_temp, hours, minutes, dhours, dminutes, heat)
    if result:
        _LOGGER.info("Режим %s с отложенным временем установлен", mode)
    else:
...
async def main():
    ...
    try:
        if args.function == 'check_connection':
            ...
            _LOGGER.error("Неизвестная функция: %s", args.function)
            ...
            _LOGGER.info("Параметры для set_mode и set_delayed_mode:")
            ...
            _LOGGER.info("Параметры для parse_status_string:")
