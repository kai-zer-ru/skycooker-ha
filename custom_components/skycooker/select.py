...
from .utils import get_base_name, get_temperature_options, get_entity_name, build_entity_id
from .time import get_time_options, _validate_hours, _validate_minutes
from .programs import (get_favorite_programs, get_favorites_other_label, get_program_options,
...
class SkyCookerSelect(SkyCookerEntity, SelectEntity):
    ...
    def __init__(self, hass, entry, select_type: str) -> None:
        ...
        self.entity_id = build_entity_id(SELECT_DOMAIN, hass, entry, select_type)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if self.select_type in [SELECT_TYPE_COOKING_TIME_HOURS, SELECT_TYPE_COOKING_TIME_MINUTES,
                                SELECT_TYPE_DELAYED_START_HOURS, SELECT_TYPE_DELAYED_START_MINUTES,
    ...
    def name(self) -> str:
        ...
        if self.select_type == SELECT_TYPE_PROGRAM:
            ...
            return get_entity_name(self.hass, self.entry, self.select_type, 'Cooking time (hours)',
                                   'Время приготовления (часы)')
        elif self.select_type == SELECT_TYPE_COOKING_TIME_MINUTES:
            return get_entity_name(self.hass, self.entry, self.select_type, 'Cooking time (minutes)',
                                   'Время приготовления (минуты)')
        elif self.select_type == SELECT_TYPE_DELAYED_START_HOURS:
            return get_entity_name(self.hass, self.entry, self.select_type, 'Delayed start (hours)',
                                   'Время до отложенного старта (часы)')
        elif self.select_type == SELECT_TYPE_DELAYED_START_MINUTES:
            return get_entity_name(self.hass, self.entry, self.select_type, 'Delayed start (minutes)',
                                   'Время до отложенного старта (минуты)')
        elif self.select_type == SELECT_TYPE_FAVORITES:
            return get_entity_name(self.hass, self.entry, self.select_type, 'Favorites', 'Избранные программы')
    ...
    def current_option(self) -> str | None:
        ...
        if self.select_type == SELECT_TYPE_PROGRAM or self.select_type == SELECT_TYPE_FAVORITES:
            ...
            if self.select_type == SELECT_TYPE_FAVORITES:
                favorites = get_favorite_programs(self.hass, self.entry, self.skycooker.model_id)
                if program_name not in favorites:
                    return get_favorites_other_label(self.hass)
            return program_name
        elif self.select_type == SELECT_TYPE_SUBPROGRAM:
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Устанавливаем значения по умолчанию для отложенного старта, если они не установлены
    ...
    async def _handle_program_selection(self, selected_program_name: str) -> None:
        ...
        if model_id is None:
            _LOGGER.warning("model_id is None, возвращаем None")
            return None
        ...
        # В селекте избранного выбор «Другое» — только обновить отображение, не менять температуру/время
        if self.select_type == SELECT_TYPE_FAVORITES and selected_program_name == get_favorites_other_label(self.hass):
            self.async_schedule_update_ha_state(True)
            async_dispatcher_send(self.hass, DISPATCHER_UPDATE)
            return None
        ...
        if program_data:
            if self.select_type == SELECT_TYPE_PROGRAM:
                _LOGGER.debug(
                    f"Выбран режим {program_id} ({selected_program_name}) для модели {model_id}: температура={program_data['temperature']}, часы={program_data['hours']}, минуты={program_data['minutes']}")
            else:
                _LOGGER.debug(
