...
class SkyCookerSensor(SkyCookerEntity, SensorEntity):
    ...
    def __init__(self, hass, entry, sensor_type):
        ...
        self.entity_id = build_entity_id(SENSOR_DOMAIN, hass, entry, sensor_type)
    ...
    def name(self):
        ...
        if self.sensor_type == SENSOR_TYPE_STATUS:
            ...
            return get_entity_name(self.hass, self.entry, self.sensor_type, 'Delayed launch time',
                                   'Время до отложенного старта')
        elif self.sensor_type == SENSOR_TYPE_CURRENT_PROGRAM:
            return get_entity_name(self.hass, self.entry, self.sensor_type, 'Current mode', 'Текущая программа')
        elif self.sensor_type == SENSOR_TYPE_SUBPROGRAM:
            return get_entity_name(self.hass, self.entry, self.sensor_type, 'Current subprogram',
                                   'Текущая подпрограмма')
        elif self.sensor_type == SENSOR_TYPE_ERROR_CODE:
            return get_entity_name(self.hass, self.entry, self.sensor_type, 'Error code', 'Код ошибки')
        elif self.sensor_type == SENSOR_TYPE_SOUND_ENABLED:
            return get_entity_name(self.hass, self.entry, self.sensor_type, 'Sound', 'Звук')
    ...
    def icon(self):
        ...
        icons = {
            ...
            SENSOR_TYPE_ERROR_CODE: "mdi:alert-circle",
            SENSOR_TYPE_SOUND_ENABLED: "mdi:volume-high",
        }
    ...
    def entity_category(self):
        ...
        if self.sensor_type in (SENSOR_TYPE_SUCCESS_RATE, SENSOR_TYPE_ERROR_CODE, SENSOR_TYPE_SOUND_ENABLED):
    ...
    def available(self):
        ...
        # Для датчиков успеха и времени отложенного старта всегда возвращаем True
        ...
        if self.sensor_type == SENSOR_TYPE_STATUS:
            ...
            return self.skycooker.status_code is not None and hasattr(self.skycooker,
                                                                      'target_additional_hours') and hasattr(
                self.skycooker, 'target_additional_minutes')
        elif self.sensor_type == SENSOR_TYPE_CURRENT_PROGRAM:
            return self.skycooker.status_code is not None
        elif self.sensor_type in (SENSOR_TYPE_ERROR_CODE, SENSOR_TYPE_SOUND_ENABLED):
            return self.skycooker.status is not None
    ...
    def native_value(self):
        ...
        if self.sensor_type == SENSOR_TYPE_STATUS:
            ...
            return 0 if self.skycooker.status_code == STATUS_OFF else (
                self.skycooker.target_temperature if self.skycooker.target_temperature is not None else 0)
        elif self.sensor_type == SENSOR_TYPE_REMAINING_TIME:
            ...
            return str(
                self.skycooker.status.subprogram_id) if self.skycooker.status and self.skycooker.status.subprogram_id is not None else "0"
        elif self.sensor_type == SENSOR_TYPE_ERROR_CODE:
            # Если ошибка не поддерживается или отсутствует, возвращаем 0
            return getattr(self.skycooker, "error_code", None)
        elif self.sensor_type == SENSOR_TYPE_SOUND_ENABLED:
            value = getattr(self.skycooker, "sound_enabled", None)
            if value is None:
                return None
            return bool(value)
