...
class SkySensor(SensorEntity):
    ...
    def available(self):
        if self.sensor_type == SENSOR_TYPE_ENERGY:
            return self.cooker.available and self.cooker.energy_wh != None
        if self.sensor_type == SENSOR_TYPE_ONTIME:
            return self.cooker.available and self.cooker.ontime != None
        if self.sensor_type == SENSOR_TYPE_HEATER_ON_COUNT:
            return self.cooker.available and self.cooker.heater_on_count != None
        if self.sensor_type == SENSOR_TYPE_USER_ON_COUNT:
            return self.cooker.available and self.cooker.user_on_count != None
        if self.sensor_type == SENSOR_TYPE_WATER_FRESHNESS:
            return self.cooker.available and self.cooker.water_freshness_hours != None
        if self.sensor_type == SENSOR_TYPE_SUCCESS_RATE:
    ...
    def native_value(self):
        if self.sensor_type == SENSOR_TYPE_ENERGY:
            return self.cooker.energy_wh
        if self.sensor_type == SENSOR_TYPE_ONTIME:
            return self.cooker.ontime.total_seconds()
        if self.sensor_type == SENSOR_TYPE_HEATER_ON_COUNT:
            return self.cooker.heater_on_count
        if self.sensor_type == SENSOR_TYPE_USER_ON_COUNT:
            return self.cooker.user_on_count
        if self.sensor_type == SENSOR_TYPE_WATER_FRESHNESS:
            return self.cooker.water_freshness_hours
        if self.sensor_type == SENSOR_TYPE_SUCCESS_RATE:
