...
| **Время до отложенного запуска** | Время до начала отложенного запуска | `sensor.skycoocker_delayed_launch_time` | - | 0 ч. 0 м., 0 ч. 30 м., 1 ч. 0 м. |
...
**Примечание**: Режим автоподогрева активируется автоматически после завершения программы приготовления, если он был включен до запуска. Это позволяет сохранить блюдо теплым до момента подачи.
...
|--------|-----------|----------|
| **Старт** | Запуск выбранной программы с текущими настройками | `button.skycoocker_start` |
| **Стоп** | Остановка текущей программы и сброс всех настроек | `button.skycoocker_stop` |
| **Отложенный старт** | Запуск программы с заданным временем отложенного старта | `button.skycoocker_start_delayed` |
...
### Версия 0.2.3 (2026-01-15)

**Улучшения:**
- ✅ Улучшена обработка времени в сенсорах
- ✅ Добавлена проверка наличия статуса и времени для расчета оставшегося времени
- ✅ Улучшена логика получения значений времени из статуса устройства
- ✅ Добавлены проверки на наличие атрибутов `target_main_hours`, `target_main_minutes`, `target_additional_hours`, `target_additional_minutes`

**Исправления:**
- ❌ Удалены дублирующиеся свойства из `skycooker_connection.py`
- ❌ Улучшена стабильность работы сенсоров при отсутствии данных
